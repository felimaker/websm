...
<?php }else{ ?>
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Tipo movimiento</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Tipo movimiento</label></div>
		...
		$j(function(){
			...
			$j("#filter_2").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movimientos", f:"tipo_movimiento" }; },
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Envio</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Envio</label></div>
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Lugar Actual</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Lugar Actual</label></div>
		...
		$j(function(){
			...
			$j("#filter_6").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movimientos", f:"origen" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Fecha movimiento</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Fecha movimiento</label></div>
	...
	<script>
		$j(function(){
			...
			$j("#from-date_7" ).on('dp.change' , function(e){
				...
				$j("#to-date_7 ").val(date.format('DD/MM/YYYY')).data("DateTimePicker").minDate(e.date);
			});
		});
	</script>
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="col-md-8 col-lg-6 vspacer-md">
			...
			<input type="hidden" id="lookupoperator_8" name="FilterOperator[6]" value="equal-to">
			<input type="hidden" id="filterfield_8" name="FilterValue[6]" value="<?php echo htmlspecialchars($FilterValue[6]); ?>" size="3">
		</div>
		...
		$j(function(){
			...
			$j("#filter_8").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movimientos", f:"destino" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Estado verifica</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Estado verifica</label></div>
		
		
		<div class="col-md-8 col-lg-6 vspacer-md">
			<div id="filter_13" class="vspacer-lg"><span></span></div>
			...
			<input type="hidden" id="lookupoperator_13" name="FilterOperator[7]" value="equal-to">
			<input type="hidden" id="filterfield_13" name="FilterValue[7]" value="<?php echo htmlspecialchars($FilterValue[7]); ?>" size="3">
		</div>
		...
		$j(function(){
			...
			$j("#filter_13").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movimientos", f:"estado_verifica" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
				$j("#filterfield_13").val(e.added.text);
				$j("#lookupoperator_13").val('equal-to');
				if (e.added.id=='{empty_value}'){
					$j("#lookupoperator_13").val('is-empty');
				}
			});


			/* preserve the applied category filter and show it when re-opening the filters page */
			if ($j("#filterfield_13").val().length){
				
				//None case 
				if ($j("#filterfield_13").val() == '<None>'){
					$j("#filter_13").select2( 'data' , {
						id: '{empty-value}',
						text: '<None>'
					});
					$j("#lookupoperator_13").val('is-empty');
				...
				$j.ajax({
					...
					dataType: 'json',
					data: {
						s: $j("#filterfield_13").val(),  //search term
						p: 1,                                         //page number
						t: "movimientos",                //table name
						f: "estado_verifica"               //field name
					}
				}).done(function(response){
					if (response.results.length){
						$j("#filter_13").select2('data' , {
