...
<?php }else{ ?>
	...
	<button class="btn btn-lg btn-default pull-right search_mode" type="submit" name="Filter_x" value="1">Switch to advanced search mode</button>
			...
			<div class="page-header"><h1>
				<a href="movi_envio_articulo_view.php" style="text-decoration: none; color: inherit;">
					<img src="resources/table_icons/arrow_right.png"> 					Envio Filters</a>
			</h1></div>

		
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Fecha envio</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Fecha envio</label></div>
		...
		$j(function(){
			...
			$j("#filter_4").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movi_envio_articulo", f:"origen" }; },
			...
			if ($j("#filterfield_4").val().length){
				...
				$j.ajax({
					...
					data: {
						...
						p: 1,                                         //page number
						t: "movi_envio_articulo",                //table name
						f: "origen"               //field name
					}
		...
		$j(function(){
			...
			$j("#filter_5").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movi_envio_articulo", f:"ubicacion" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Lugar de destino</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Lugar de destino</label></div>
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Ciudad destino</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Ciudad destino</label></div>
		...
		$j(function(){
			...
			$j("#filter_8").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movi_envio_articulo", f:"ciudad" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Responsable de envio</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Responsable de envio</label></div>
		...
		<div class="col-md-8 col-lg-6 vspacer-md">
			...
			<input type="hidden" id="lookupoperator_9" name="FilterOperator[7]" value="equal-to">
			<input type="hidden" id="filterfield_9" name="FilterValue[7]" value="<?php echo htmlspecialchars($FilterValue[7]); ?>" size="3">
		</div>
		...
		$j(function(){
			...
			$j("#filter_9").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"movi_envio_articulo", f:"nombre" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
