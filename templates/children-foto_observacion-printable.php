...
<?php $current_table = 'foto_observacion'; ?>
<?php
			...
			<table data-tablename="<?php echo $current_table; ?>" class="table table-striped table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr data-id="<?php echo html_attr($pkValue); ?>">
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][2]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][2]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><a href="<?php echo getUploadDir('') . urlencode($record[2]); ?>" data-lightbox="foto_observacion-foto"><img src="thumbnail.php?i=<?php echo urlencode($record[2]); ?>&t=foto_observacion&f=foto&v=tv" class="img-thumbnail"></a></td>
