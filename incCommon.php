	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'marca_presetacion' => ['Modelo presetacion', '', 'resources/table_icons/cog_add.png', 'ARTICULOS'],
			...
			'ubicacion_articulo' => ['Ubicacion articulo', '', 'resources/table_icons/building.png', 'ARTICULOS'],
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'ubicacion_articulo' => "`ubicacion_articulo`.`id` as 'id', IF(    CHAR_LENGTH(`articulos1`.`nombre_unico`) || CHAR_LENGTH(`marcas1`.`marca`), CONCAT_WS('',   `articulos1`.`nombre_unico`, ' ', `marcas1`.`marca`), '') as 'id_articulo', IF(    CHAR_LENGTH(`unidades1`.`nombre`), CONCAT_WS('',   `unidades1`.`nombre`), '') as 'ubicacion', `ubicacion_articulo`.`nota` as 'nota', if(`ubicacion_articulo`.`creado`,date_format(`ubicacion_articulo`.`creado`,'%d/%m/%Y %h:%i %p'),'') as 'creado', `ubicacion_articulo`.`creado_por` as 'creado_por'",
			'ubicacion_mobiliario' => "`ubicacion_mobiliario`.`id` as 'id', IF(    CHAR_LENGTH(`mobiliario1`.`nombre`) || CHAR_LENGTH(`tipo_mobiliario1`.`tipo_mobiliario`), CONCAT_WS('',   `mobiliario1`.`nombre`, ' ', `tipo_mobiliario1`.`tipo_mobiliario`), '') as 'id_mobiliario', IF(    CHAR_LENGTH(`unidades1`.`nombre`) || CHAR_LENGTH(`unidades1`.`abreviado`), CONCAT_WS('',   `unidades1`.`nombre`, ' ', `unidades1`.`abreviado`), '') as 'ubicacion', if(`ubicacion_mobiliario`.`creado`,date_format(`ubicacion_mobiliario`.`creado`,'%d/%m/%Y %h:%i %p'),'') as 'creado', `ubicacion_mobiliario`.`creado_por` as 'creado_por', `ubicacion_mobiliario`.`nota` as 'nota'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'cofig' => "`cofig` ",
			'marca_presetacion' => "`marca_presetacion` LEFT JOIN `marca_modelo` as marca_modelo1 ON `marca_modelo1`.`id_mmodelo`=`marca_presetacion`.`modelo` LEFT JOIN `tipo_unidad_medida` as tipo_unidad_medida1 ON `tipo_unidad_medida1`.`id`=`marca_presetacion`.`unidad_medida` ",
			...
			'ubicacion_articulo' => "`ubicacion_articulo` LEFT JOIN `articulos` as articulos1 ON `articulos1`.`id`=`ubicacion_articulo`.`id_articulo` LEFT JOIN `marcas` as marcas1 ON `marcas1`.`id_marca`=`articulos1`.`marca` LEFT JOIN `unidades` as unidades1 ON `unidades1`.`id_unidades`=`ubicacion_articulo`.`ubicacion` ",
