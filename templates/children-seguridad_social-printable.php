...
<?php $current_table = 'seguridad_social'; ?>
<?php
			...
			<table data-tablename="<?php echo $current_table; ?>" class="table table-striped table-hover table-condensed table-bordered">
				...
				<tbody>
					...
					<tr data-id="<?php echo html_attr($pkValue); ?>">
						...
						<td class="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][3]}"; ?>" id="<?php echo "{$parameters['ChildTable']}-{$config['display-field-names'][3]}-" . html_attr($record[$config['child-primary-key-index']]); ?>"><?php if($record[3]) { ?><a href="link.php?t=<?php echo $parameters['ChildTable']; ?>&f=docuemento&i=<?php echo urlencode($record[$config['child-primary-key-index']]); ?>" target="_blank" class="btn btn-default" title="<?php echo html_attr($record[3]); ?>"><i class="glyphicon glyphicon-lg glyphicon-picture text-info"></i></a><?php } ?></td>
