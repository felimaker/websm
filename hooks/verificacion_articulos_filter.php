...
<?php }else{ ?>
	...
	<button class="btn btn-lg btn-default pull-right search_mode" type="submit" name="Filter_x" value="1">Switch to advanced search mode</button>
			...
			<div class="page-header"><h1>
				<a href="verificacion_articulos_view.php" style="text-decoration: none; color: inherit;">
					<img src="resources/table_icons/accept.png"> 					Verificacion Filters</a>
			</h1></div>
			...
			if ($j("#filterfield_2").val().length){
				...
				$j.ajax({
					...
					data: {
						...
						p: 1,                                         //page number
						t: "verificacion_articulos",                //table name
						f: "codigo"               //field name
					}
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Tipo de dispositivo</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Tipo de dispositivo</label></div>
		...
		$j(function(){
			...
			$j("#filter_3").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"verificacion_articulos", f:"tipo_dispo" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
		...
		$j(function(){
			...
			$j("#filter_4").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"verificacion_articulos", f:"nombre" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
		...
		$j(function(){
			...
			$j("#filter_5").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"verificacion_articulos", f:"serial" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;">
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Abreviado</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Abreviado</label></div>
		...
		$j(function(){
			...
			$j("#filter_7").select2({
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ return { s: term, p:page, t:"verificacion_articulos", f:"abreviado" }; },
					results: function (resp, page) { return resp; }
				}
			}).on('change', function(e){
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Fecha de verificado</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Fecha de verificado</label></div>
		...
		<div class="col-xs-9 col-md-3 col-lg-2 vspacer-md">
			<input type="text"  class="form-control" id="from-date_10"  name="FilterValue[7]" value="<?php echo htmlspecialchars($FilterValue[7]); ?>" size="10">
		</div>
		...
		<div class="col-xs-9 col-md-3 col-lg-2 vspacer-md">
			<input type="text" class="form-control" id="to-date_10" name="FilterValue[8]" value="<?php echo htmlspecialchars($FilterValue[8]); ?>" size="10">
		</div>
	...
	<script>
		$j(function(){
			//date
			$j("#from-date_10 , #to-date_10 ").datetimepicker({
				format: 'YYYY-MM-DD HH:mm:ss'   //config
			});

			$j("#from-date_10" ).on('dp.change' , function(e){
				date = moment(e.date).add(1, 'month');  
				$j("#to-date_10 ").val(date.format('YYYY-MM-DD HH:mm:ss')).data("DateTimePicker").minDate(e.date);
			});
		});
	</script>
