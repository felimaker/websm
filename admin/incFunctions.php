		...
		if($schema === null) {
			...
			$schema = [
				...
				'articulos' => [
					...
					'modelo' => ['appgini' => "INT UNSIGNED NULL"],
					'presentacion' => ['appgini' => "INT UNSIGNED NULL"],
					'serial' => ['appgini' => "VARCHAR(40) NULL"],
		...
		*/
		$lookupQuery = [
			...
			'financiero_mobiliario' => [
				...
				'unidad' => 'SELECT `mobiliario`.`id_mobiliario`, IF(CHAR_LENGTH(`mobiliario`.`unidad`), CONCAT_WS(\'\', IF(    CHAR_LENGTH(`unidades1`.`nombre`), CONCAT_WS(\'\',   `unidades1`.`nombre`), \'\'), \' - \'), \'\') FROM `mobiliario` LEFT JOIN `tipo_grupo_mobilia` as tipo_grupo_mobilia1 ON `tipo_grupo_mobilia1`.`id_gru_mo`=`mobiliario`.`grupo` LEFT JOIN `tipo_mobiliario` as tipo_mobiliario1 ON `tipo_mobiliario1`.`id`=`mobiliario`.`tipo_mobiliario` LEFT JOIN `unidades` as unidades1 ON `unidades1`.`id_unidades`=`mobiliario`.`unidad` ORDER BY 2',
			],
			'prestador' => [
