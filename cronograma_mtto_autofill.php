...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	$table_perms = getTablePermissions('cronograma_mtto');
	...
	switch($mfk) {
		...
		case 'id_programa':
			if(!$id) {
				?>
				$j('#ano<?php echo $rnd1; ?>').html('&nbsp;');
				<?php
				break;
			}
			$res = sql("SELECT `programa_mtto`.`id` as 'id', IF(    CHAR_LENGTH(`unidades1`.`nombre`), CONCAT_WS('',   `unidades1`.`nombre`), '') as 'unidad', `programa_mtto`.`ano` as 'ano', `programa_mtto`.`descripcion` as 'descripcion', `programa_mtto`.`enlace` as 'enlace', `programa_mtto`.`archivo` as 'archivo', if(`programa_mtto`.`creado`,date_format(`programa_mtto`.`creado`,'%d/%m/%Y %h:%i %p'),'') as 'creado', `programa_mtto`.`creado_por` as 'creado_por' FROM `programa_mtto` LEFT JOIN `unidades` as unidades1 ON `unidades1`.`id_unidades`=`programa_mtto`.`unidad`  WHERE `programa_mtto`.`id`='{$id}' limit 1", $eo);
			$row = db_fetch_assoc($res);
			?>
			$j('#ano<?php echo $rnd1; ?>').html('<?php echo addslashes(str_replace(array("\r", "\n"), '', nl2br($row['ano']))); ?>&nbsp;');
			<?php
			break;
