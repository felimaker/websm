...
<?php
	...
	if(is_array($arrTables) && count($arrTables)) {
		...
		foreach($tg as $tn => $tgroup) {
			...
			$tChkHL = array_search($tn, array('contactos_tipo_contacto','marcas','accesorios','hojadevida','financiero','dispositivo_tipo_grupo','tipo_relacion','tipo_dispositivo','tipo_documento','tipo_iden','tipo_razon_social','tipo_mobiliario','tipo_estado_dispo','tipo_movimiento','tipo_estado_movi','tipo_estado_verifica','tipo_mtto','tipo_calibracion','dispo_detalles_tecnicos','tipo_documento_ss','tipo_parentesco','tipo_rh','consentimieto','perfil_familia','perfil_soportes','seguridad_social','ubicacion_articulo','ubicacion_mobiliario'));
			/* allow homepage 'add new' for current table? */
			$tChkAHAN = array_search($tn, array('dispositivos','compra_dispo'));
