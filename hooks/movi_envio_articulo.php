<?php
	...
	/**
	 * @file
	 * This file contains hook functions that get called when data operations are performed on 'movi_envio_articulo' table. 
	...
	*/
	...
	function movi_envio_articulo_init(&$options, $memberInfo, &$args) {
		/* Inserted by Search Page Maker for AppGini on 2021-08-02 10:38:29 */
		$options->FilterPage = 'hooks/movi_envio_articulo_filter.php';
	...
	*/
	...
	function movi_envio_articulo_header($contentType, $memberInfo, &$args) {
		$header='';
	...
	*/
	...
	function movi_envio_articulo_footer($contentType, $memberInfo, &$args) {
		$footer='';
	 ...
	 *     $data['fecha_envio'], $data['origen'], $data['ubicacion'], $data['destino'], $data['d_direccion'], $data['ciudad'], $data['nombre'], $data['telefono'], $data['detalles']
	...
	*/
	...
	function movi_envio_articulo_before_insert(&$data, $memberInfo, &$args) {
	 ...
	 * For this table, the array items are: 
	 *     $data['id_movi'], $data['fecha_envio'], $data['origen'], $data['ubicacion'], $data['destino'], $data['d_direccion'], $data['ciudad'], $data['nombre'], $data['telefono'], $data['detalles'], $data['creado'], $data['creado_por'], $data['editado'], $data['editado_por']
	...
	*/
	...
	function movi_envio_articulo_after_insert($data, $memberInfo, &$args) {
	 ...
	 * Note: if a field is set as read-only or hidden in detail view, it can't be modified through $data. You should use a direct SQL statement instead.
	 * For this table, the array items are: 
	 *     $data['id'], $data['fecha_envio'], $data['origen'], $data['ubicacion'], $data['destino'], $data['d_direccion'], $data['ciudad'], $data['nombre'], $data['telefono'], $data['detalles']
	...
	*/
	...
	function movi_envio_articulo_before_update(&$data, $memberInfo, &$args) {
	 ...
	 * An associative array where the keys are field names and the values are the field data values.
	 * For this table, the array items are: 
	 *     $data['id'], $data['id_movi'], $data['fecha_envio'], $data['origen'], $data['ubicacion'], $data['destino'], $data['d_direccion'], $data['ciudad'], $data['nombre'], $data['telefono'], $data['detalles'], $data['creado'], $data['creado_por'], $data['editado'], $data['editado_por']
	...
	*/
	...
	function movi_envio_articulo_after_update($data, $memberInfo, &$args) {
	...
	*/
	...
	function movi_envio_articulo_before_delete($selectedID, &$skipChecks, $memberInfo, &$args) {
	...
	*/
	...
	function movi_envio_articulo_after_delete($selectedID, $memberInfo, &$args) {
	...
	*/
	...
	function movi_envio_articulo_dv($selectedID, $memberInfo, &$html, &$args) {
	...
	*/
	...
	function movi_envio_articulo_csv($query, $memberInfo, &$args) {
	...
	*/
	...
	function movi_envio_articulo_batch_actions(&$args) {
