...
<?php
	...
	$AGENDA_from = get_sql_from('agenda');
	if(!$AGENDA_from) exit(error_message('Acceso denegado!', false));
	
	/* get invoice */
	$AGENDA_id = intval($_REQUEST['id_agenda']);
	if(!$AGENDA_id) exit(error_message('ID Invalido!', false));
	
	$AGENDA_fields = get_sql_fields('agenda');
	$res = sql("select {$AGENDA_fields} from {$AGENDA_from} and id_agenda={$AGENDA_id}", $eo);
...
?>	
	<div class="row">
		<div>
			...
			<td style="width: 20%; height: 21px;"><strong>NOMBRE:</strong> <?php echo $AGENDA['nombre']?></td>
			<td style="width: 30%; height: 21px;"><strong>FECHA:</strong> <?php echo $AGENDA['date']?></td>
			</tr>
			<tr style="height: 21px;">
