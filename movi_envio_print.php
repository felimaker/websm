...
<?php
	...
	$AGENDA_from = get_sql_from('movi_envio');
	if(!$AGENDA_from) exit(error_message('Acceso denegado!', false));
	
	/* get invoice */
	$AGENDA_id = intval($_REQUEST['id']);
	if(!$AGENDA_id) exit(error_message('ID Invalido!', false));
	
	$AGENDA_fields = get_sql_fields('movi_envio');
	$res = sql("select {$AGENDA_fields} from {$AGENDA_from} and id={$AGENDA_id}", $eo);
	if(!($AGENDA = db_fetch_assoc($res))) exit(error_message('Registro no encontrado!', false));
?>	
	<div class="row">
		<div>
			<p><img src="images/HTU.png" alt="" width="85" height="79" />&nbsp;<img src="images/starmedica-2.png" alt="" width="163" height="60" /></p>
			...
			<td style="width: 20%; height: 21px;"><strong>ORIGEN:</strong> <?php echo $AGENDA['origen']?></td>
			<td style="width: 30%; height: 21px;"><strong>FECHA ENVIO:</strong> <?php echo $AGENDA['fecha_envio']?></td>
			</tr>
			<tr style="height: 21px;">
